/**
 * 24. Create a separate countries.js file and store
 * the countries array in to this file, create a separate file web_techs.js
...
/* const store={
    ...
    modifyTea:function(){
        ...
        if(index !==-1){
            this.shoppingCart.slice(index,1,"Green Tea");
        }
    },
};
...
const studentAges = {
    ...
    averageAge: function (){
        const sum =this.ages.reduce((prevValue, currentValue) => prevValue + currentValue,0);
 ...
 */
 ...
 console.log(`Random Hexadecimal number ${getRandom()}`);
 ...
 */
    ...
    //1
    const filcoun=countries.filter((country)=>{
    ...
    //2
    let highestchar= countries[0].length;
    for (let country of countries){
        if (country.length > highestchar){
            highestchar=country.length;
        }
    }
